...
class SampleCount:
    ...
    def get_total_count(self) -> list:
        out = []
        for i in range(len(self._h1_total_count)):
            count = self._h1_total_count[i] + self._h2_total_count[i] + self._un_total_count[i]
            out.append(count)
        return out
    def get_length(self):
...
class MethylSites:
    ...
    def add_sample_count(self, sample:str, count:dict):
        self._sample_count[sample] = SampleCount(count.get("methyl").get("h1"),count.get("total").get("h1"),
                                                 count.get("methyl").get("h2"),count.get("total").get("h2"),
                                                 count.get("methyl").get("un"),count.get("total").get("un"))
...
class Bed:
    ...
    def is_in_id_region(self, id:str, chr:str, pos:int) -> bool:
        ...
        return False
    def __str__(self):
        return str({"ID":self._id, "CHR": self._chr, "START": self._start, "END": self._end})
    ...
    with open(file, 'r') as f:
        ...
        for line in f.readlines():
            ...
            if current_gene != ss[2]:
                if current_gene != "":
                    methyl_dict[current_gene] = MethylSites(current_gene, current_chr, sorted(current_pos_set), dict())
                current_gene = ss[2]
                current_chr = ss[3]
        ...
        for line in f.readlines():
            ...
            if current_gene != ss[2]:
                if first:
                    ...
                    print("       finish", current_sample, current_gene, len(current_pos_list))
                ...
                current_count = { "methyl": { "h1": [0]*len(current_pos_list),
                                              "h2": [0]*len(current_pos_list),
                                              "un": [0]*len(current_pos_list)},
                                   "total": { "h1": [0]*len(current_pos_list),
                                              "h2": [0]*len(current_pos_list),
                                              "un": [0]*len(current_pos_list)} }
            elif ss[2] != one_gene:
                pass
...
def draw_heatmap(methyl_dict:dict, gene:str):
    ...
    df = pd.DataFrame(dic)
    sns.clustermap(df.T, cmap="coolwarm", vmin=0.0, vmax=1.0, row_cluster=True, col_cluster=True)
